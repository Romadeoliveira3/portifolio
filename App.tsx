...
const App: React.FC = () => {
  ...
  const desktopIcons = useMemo(() => {
    return DESKTOP_ICONS_CONFIG.map(iconConfig => ({
      ...iconConfig,
      label: translations.desktop_icons[iconConfig.id as keyof typeof translations.desktop_icons],
    }));
  }, [translations]);
  ...
  return (
    <div className="w-screen h-screen bg-cover bg-center bg-[url('https://images.unsplash.com/photo-1531297484001-80022131f5a1?q=80&w=1920&h=1080&auto=format&fit=crop')] font-sans overflow-hidden">
      {showInstaller && <InstallerWizard onFinish={handleInstallerFinish} isMobile={isMobile} />}
      <Desktop
        icons={desktopIcons}
